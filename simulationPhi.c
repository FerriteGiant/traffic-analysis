...
int trackLength;
int sampleRate = 1;
int dataStartStep = 100000; //5e6
int numSteps,numSamples;
int fftSamples=1; //Time steps per FFT
...
if (argc != 7)
{
  printf("Usage: %s <numDataPoints> <trackLength> <numSteps> <density> <maxVel> <save directory>\n",argv[0]);
  return(-1);
}
else
{ //First argument
  ...
  fftRuns = 2;//numDataPoints/(numCars*fftSamples);
  ...
  asprintf(&fileName, "%sphi0_vel%d_density%.4f_track%d"
                          "_steps%d_numCars%d.csv",\
                          argv[6],maxVel,density,trackLength,\
                          numSamples,numCars);
  printf("%s\n",fileName);
...
//  for (i=0;i<fftSamples;i++){
  ...
  for (t=0;t<numSamples;t++){
    ...
    phi(t,posArray, velArray,phiArray,phi0Array);
...
//Print phi0Array
  fprintf(outputFile,"numCars,%d,maxVel,%d,trackLength,%d,"
                    "numSteps,%d\n",\
                    numCars,maxVel,trackLength,\
                    numSamples);
for (t=0;t<numSamples;t++){
  if(t<numSamples-1)
    fprintf(outputFile,"%d,",phi0Array[t]);
  else
    fprintf(outputFile,"%d",phi0Array[t]);
}
...
void pairCorrelation(int *posArray, int *velArray, int *distArray)
{
  ...
  int i,j,dist;
    ...
    //Append first car to end of array for lookahead purposes
